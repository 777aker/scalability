...
var head_pos: Vector2 = Vector2(255, 250) # 255 so doesn't collide with edges of screen
...
func spawn_segment() -> void:
	...
	if head_pos.x < 0 or head_pos.y < 36 or head_pos.x > 500 or head_pos.y > 520:
		game_over()
		return
...
func spawn_apple() -> void:
	var apple_pos_x = randi_range(0, 480 / GRID_SIZE) * GRID_SIZE + 15 # has a little offset from edge of screen
	var apple_pos_y = randi_range(0, 480 / GRID_SIZE) * GRID_SIZE + 50 # has a little offset from title bar
